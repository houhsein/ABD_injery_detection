...
GlobalParams = collections.namedtuple('GlobalParams', [
    ...
    'drop_connect_rate', 'depth_divisor', 'min_depth', 'include_top', 'normal'])
...
def round_filters(filters, global_params):
    """Calculate and round number(四捨五入) of filters based on width multiplier.
...
def round_repeats(repeats, global_params):
    ...
    if not multiplier:
        return repeats
    # follow the formula transferred from official TensorFlow implementation
    # math.ceil return round number(int)
...
def efficientnet3d(width_coefficient=None, depth_coefficient=None, image_size=None,
                 dropout_rate=0.2, drop_connect_rate=0.2, num_classes=1000, include_top=True, normal=True):
...
def get_model_params(model_name, normal, override_params):
...
#         這裡為默認尺寸 根據 efficientnet_params內定義各自的參數
        w, d, s, p = efficientnet_params(model_name)
        # note: all models have drop connect rate = 0.2
        blocks_args, global_params = efficientnet3d(
            width_coefficient=w, depth_coefficient=d, dropout_rate=p, image_size=s, normal=normal)
    else:
        raise NotImplementedError('model name is not pre-defined: {}'.format(model_name))
    if override_params:
#         若是在之前有先定義 override_params 將會覆蓋掉預設參數
