...
class ResCBAMLayer(nn.Module):
    def __init__(self, in_planes, feature_size, reduction=4):
        ...
        self.ch_Linear1 = nn.Linear(in_planes, in_planes // reduction, bias=False)
        self.ch_Linear2 = nn.Linear(in_planes // reduction, in_planes, bias=False)
...
class ConvRes(nn.Module):
    def __init__(self, input_size, config, normal=False, input_channel=1, num_classes = 2):
        super(ConvRes, self).__init__()
        self.conv1 = conv3d_same_size(in_channels=input_channel, out_channels=4, kernel_size=3)
        self.conv2 = conv3d_same_size(in_channels=4, out_channels=4, kernel_size=3)
        self.config = config
        self.last_channel = 4
        self.first_cbam = ResCBAMLayer(4, input_size)
        self.normal = normal
        ...
        for stage in config:
            ...
            layers.append(ResCBAMLayer(self.last_channel, input_size//(2**i)))
        self.layers = nn.Sequential(*layers)
        # input_size 等於kernel size，所以不管stride多少，都全部濃縮成(1,1,1)
        self.avg_pooling = nn.AvgPool3d(kernel_size=input_size//8, stride=4)
        if normal:
            self.fc = nn.Linear(512,self.num_classes)
        else:
            self.fc = AngleLinear(in_features=self.last_channel, out_features=self.num_classes)
