...
# Based on https://github.com/ljwztc/CLIP-Driven-Universal-Model/blob/main/model/Universal_model.py
...
class SwinUNETRClassifier(nn.Module):
    def __init__(self, img_size, in_channels, out_channels, encoding='rand_embedding'):
        ...
        self.backbone = SwinUNETR(img_size=img_size,
                                      ...
                                      use_checkpoint=False)
        self.GAP = nn.Sequential(
                nn.GroupNorm(16, 768),  # 使用768是因为SwinViT最后一層的输出通道數
                nn.ReLU(inplace=True),
                nn.AdaptiveAvgPool3d((1, 1, 1)),  # 全局池化，将特徵圖缩减到1x1x1
                nn.Conv3d(768, 256, kernel_size=1, stride=1, padding=0)  # 通过1x1卷積调整通道數
            )
    ...
    def forward(self, x_in):
        ...
        # 將任務編碼加到特徵向量上
        x_feat = x_feat + task_encoding.mean(dim=0)

        out = self.classifier(x_feat)
        return out

class SwinUNETRClassifierANDSegmentation(nn.Module):
    def __init__(self, img_size, in_channels, out_channels, encoding='rand_embedding'):
        super(SwinUNETRClassifier, self).__init__()
        self.backbone = SwinUNETR(img_size=img_size,
                                      in_channels=in_channels,
