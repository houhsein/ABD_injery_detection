...
class Resnet3D_3_input(nn.Module):
    def __init__(self, size, num_classes, device, normal=True):
        super(Resnet3D_3_input, self).__init__()
        # num_classes不重要，都會重新取在linear前進行BiFPN
        self.resnet = resnet101(sample_input_W=size[0], sample_input_H=size[1], sample_input_D=size[2], num_seg_classes=num_classes, shortcut_type='B')
        self.device = device
        self.load_weights()
        # 接 FPN 才需要 
